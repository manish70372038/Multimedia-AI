  ...
  return (
    <div style={s.page}>
      <motion.div style={s.hero} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 style={s.title}>Upload & Understand<br /><span style={{ color: 'var(--accent)' }}>Any Media.</span></h1>
        <p style={s.sub}>PDF, audio, or video — instant AI summaries, timestamps, and a chatbot.</p>
      </motion.div>

      <motion.div
        style={{ ...s.card, ...s.zone, borderColor: drag ? 'var(--accent)' : status === 'done' ? 'var(--green)' : 'var(--border)', borderStyle: status === 'done' ? 'solid' : 'dashed', background: drag ? 'rgba(124,110,255,0.05)' : 'var(--bg3)' }}
        onDragOver={(e) => { e.preventDefault(); setDrag(true) }}
        onDragLeave={() => setDrag(false)}
        onDrop={onDrop}
        onClick={() => !file && inputRef.current.click()}
      >
        <input ref={inputRef} type="file" accept=".pdf,.mp3,.wav,.mp4,.mkv" onChange={e => { if (e.target.files[0]) pick(e.target.files[0]) }} style={{ display: 'none' }} />
        <AnimatePresence mode="wait">
          {status === 'done' ? (
            <motion.div key="done" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '16px 0' }}>
            ...
            <motion.div key="file" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', alignItems: 'center', gap: 16, textAlign: 'left', padding: '8px 0', cursor: 'default' }}>
              ...
              <button style={{ ...s.btn, ...s.ghost, padding: '7px 14px', fontSize: 13 }} onClick={e => { e.stopPropagation(); setFile(null); setResult(null); setStatus('idle') }}>Change</button>
            </motion.div>
          ) : (
      ...
      {status === 'uploading' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ marginBottom: 16 }}>
          ...
          <div style={{ height: 5, background: 'var(--bg2)', borderRadius: 99, overflow: 'hidden' }}>
            <motion.div animate={{ width: `${progress}%` }} style={{ height: '100%', background: 'linear-gradient(90deg,var(--accent),var(--accent2))', borderRadius: 99 }} />
          </div>
        </motion.div>
      )}
      ...
      {file && status !== 'done' && (
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 32 }}>
          <button style={{ ...s.btn, ...s.primary, opacity: status === 'uploading' ? 0.6 : 1 }} onClick={handleUpload} disabled={status === 'uploading'}>
            {status === 'uploading' ? <><Loader size={16} className="spin" /> Processing...</> : <>Process File <ChevronRight size={16} /></>}
          </button>
        </div>
      )}
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {(result.type === 'audio' || result.type === 'video') && <VideoTrans file={file} mediaRef={mediaRef} />}
          <div style={s.tabs}>
            {[{ key: 'summary', label: 'Summary' }, ...(result.type !== 'pdf' ? [{ key: 'timestamps', label: 'Timestamps' }] : [])].map(({ key, label }) => (
              <button key={key} style={{ ...s.tab, background: tab === key ? 'var(--accent)' : 'transparent', color: tab === key ? '#fff' : 'var(--tx2)' }} onClick={() => setTab(key)}>{label}</button>
            ))}
          </div>
          ...
          <div style={{ ...s.card, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, background: 'linear-gradient(135deg,var(--bg3),var(--bg2))' }}>
            <div>
              ...
              <p style={{ color: 'var(--tx2)', fontSize: 13 }}>Chat is now active based on your content.</p>
            </div>
            <a href="/chat" style={{ ...s.btn, ...s.primary, textDecoration: 'none' }}>Open Chat <ChevronRight size={16} /></a>
          </div>
        </motion.div>
      )}
